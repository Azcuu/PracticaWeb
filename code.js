...
async function añadirEquipo(pokemonData) {
    ...
    if (miEquipo.some(p => p.name === pokemonData.name)) {
        alert(`${pokemonData.name} ya está en tu equipo.`);
        return true;
    }
...
function eliminarPokemon(nombre) {
    // Filtrar el equipo quitando el pokémon clicado
    const index = miEquipo.findIndex(p => p.name === nombre);
    if (index !== -1) {
        ...
        const botones = document.querySelectorAll('.botonPokemon');
            botones.forEach(boton => {
                const cardName = boton.parentElement.querySelector('h3').innerText;
                if (cardName === nombre) {
...
async function fetchPokemons() {
    const response = await fetch(`${url}pokemon/?limit=99&offset=200`);
